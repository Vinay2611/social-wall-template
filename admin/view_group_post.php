...
$post_id = $_GET['id'];
if (!empty($post_id)){
    $sql = "SELECT u.first_name,u.last_name,u.email_id,up.image,up.caption,up.status,up.user_post_id,up.group_id FROM `users_post` AS up JOIN  users AS u on up.user_id = u.user_id WHERE u.status = 'Active' AND user_post_id = '$post_id'";
    $que = $db2->query( $sql );
    if($que->num_rows > 0) {
        $i = 0;
